import React, { useRef, useEffect, FC, MouseEvent, useMemo } from 'react';
...
export const EChartsComponent: FC<ChartProps> = React.memo(({ widget, data, onElementClick, onElementContextMenu }) => {
    ...
    // Use useMemo for derived data to ensure stability
    const labels = useMemo(() => isChartData ? (data as any).labels : [], [isChartData, data]);
    const datasets = useMemo(() => isChartData ? (data as any).datasets : [], [isChartData, data]);
    ...
    const allValuePills = useMemo(() => [...(widget.shelves?.values || []), ...(widget.shelves?.values2 || [])], [widget.shelves]);
    ...
    const options = useMemo(() => {
                ...
                switch (chartType) {
                    ...
                    case ChartType.PIE:
                        chartSpecificProps = {
                            ...
                            data: labels.map((l: any, j: number) => ({ name: l, value: ds.data[j] })),
                        };
                        break;
        ...
        return options;
    }, [widget, data, themeConfig, chartColors, allValuePills, isCartesian, isHorizontal, labels, datasets, chartType, colorPalette, isStacked, chartSettings]);
    ...
    }), [onElementClick, onElementContextMenu]);
